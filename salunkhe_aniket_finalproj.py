...
Original file is located at
    https://colab.research.google.com/drive/1TAaFSkwSJoVub_6IfNVnyLeq6RjNRSbL

# Mushroom Binary Classification
## Name: Aniket Nitin Salunkhe
## UCID: as4593

### Dataset link: https://www.kaggle.com/datasets/prishasawhney/mushroom-dataset?select=mushroom_cleaned.csv
### GitHub Link: https://github.com/AniketSalunkheNJIT/salunkhe-aniket-data-mining-finalProj
"""
...
"""## Long Short-Term Memory (LSTM) Classification with K-Fold Cross-Validation

We train and evaluate a **Long Short-Term Memory (LSTM)** deep learning model using **10-Fold Cross-Validation**. This method ensures robust evaluation by splitting the dataset into 10 subsets (folds) and rotating the training and validation sets across folds.

### Process

1. **Data Preparation:**
   - The input features are reshaped to match the LSTM model's expected input format (3D: samples, timesteps, features).
   - `X_train` and `X_test` are expanded along the last dimension to prepare the data for sequential processing by LSTM layers.

2. **LSTM Model Design:**
   - The LSTM architecture includes:
     - A **Bidirectional LSTM** layer to capture both forward and backward dependencies in the data.
...
3. **K-Fold Cross-Validation:**
   - The dataset is split into 10 folds using `StratifiedKFold`, ensuring class balance in each fold.
   - For each fold:
     - Training and validation subsets are created.
     - The LSTM model is trained using **early stopping** to prevent overfitting.
     - Predictions (`y_pred_k`) and probability scores (`y_prob_k`) are generated for the validation subset.

4. **Metric Calculation:**
   - Performance metrics are computed for each fold using the `calculate_metrics` function. These include:
     - **Accuracy**
     - **Precision**
     - **Recall**
     - **F1-Score**
     - **Brier Score**
     - And other derived metrics
   - Metrics for each fold are stored in a list for analysis.

5. **Metrics Aggregation:**
   - The metrics for all folds are consolidated into a DataFrame.
   - The average values of the metrics are calculated to summarize the overall performance of the LSTM model.

### Output

- **Fold-wise Metrics:** Displays detailed performance metrics for each fold, showing the variability in the LSTM model's performance across different subsets of the data.
- **Average Metrics:** Presents the mean values of all metrics across the 10 folds, providing a comprehensive evaluation of the LSTM model.

### Observations

The **LSTM model** is capable of capturing complex sequential patterns and relationships in the data. It performs particularly well when there is temporal or positional significance in the features. The evaluation includes standard classification metrics and probability calibration metrics like the **Brier Score**, ensuring a well-rounded assessment of the model's effectiveness.
...
Among the evaluated models, **Random Forest (RF)** emerges as the best performer for several reasons:
1. **Accuracy and Robustness**: Random Forest achieves the highest accuracy among the tested models, demonstrating its ability to handle the complexity and non-linear relationships in the mushroom dataset.
2. **Feature Importance**: It provides insights into feature importance, helping identify which mushroom characteristics are most predictive for classification.
3. **Resistance to Overfitting**: By aggregating the results of multiple decision trees, Random Forest reduces the risk of overfitting, even with complex datasets.
4. **Versatility**: It performs well across a wide range of data distributions and can handle both numerical and categorical features effectively.

The combination of high performance, interpretability, and robustness makes Random Forest the most reliable and practical choice for this binary classification task.
"""
